...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * The main books, out in the open.
     *
     * These used to live in the overflow menu, which meant a shopkeeper could
     * use the app for weeks without ever learning that the Cashbook, the cheque
     * register, or payment plans existed. A feature nobody can find is a
    ...
    private fun setupBottomNav() {
        ...
        nav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.nav_cashbook -> {
                    startActivity(Intent(this, CashbookActivity::class.java))
                    false  // don't leave the tab selected; we're coming back
                }
    ...
    /** Everything that is not an everyday book: settings, tools, safety. */
    private fun showMoreSheet() {
        val options = arrayOf(
            getString(R.string.zakat_calculator),
            getString(R.string.business_settings),
    ...
    override fun onResume() {
        super.onResume()
        // Coming back from another screen, Khata is where we are.
        findViewById<BottomNavigationView>(R.id.bottomNav)?.menu
            ?.findItem(R.id.nav_khata)?.isChecked = true
    }
}
